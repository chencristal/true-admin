...
$("title").append("TrueService | Agent");
...
function mainFunction(callback){  
  ...
  // If there are no problems
    ...
    $.post('/dataAgent/', {start: date_start, end: date_end, callcenter:ids_callcenter, campaign: ids_campaign}).fail().done(function(response){
      if (response != undefined){
        ...
        if (response.count_match == 0) {
          ...
          dbWords(); 
          var countm = response.count_match;
          var count_transc = response.count_transc;

          console.log('count_transc', count_transc)
          
          words = response.words;
          countw = response.countw;          
          topWord( words, countw, countm);
          sentiments((response.pos).toFixed(2), (response.neg).toFixed(2), countm);
          var names_agent = response.names_agent;          

          var valor = parseFloat('0.0');
          var div = parseFloat('1.0');

          if ((response.match).length != 0){
            for(var x in response.match){
              valor+=parseFloat(response.match[x]);
            }
            div = parseFloat((response.match).length);
            var result = (valor/div).toFixed(2);
          }
          else{
            var result = 0.0;
          }          
          
          matchPopUp(names_agent, response.match, date_start, date_end);
          matchNumber(result, countm, count_transc);
...
function searchWords(){
  ...
  // get response search
  $.post('/searchAgent/', {start: date_start, end: date_end, words: words,callcenter:ids_callcenter, campaign: ids_campaign}).done(function(response){
    if (response != undefined){
...
function dbWords(){ 
  ...
  $.post('/selectAgent/', {start: date_start, end: date_end, idscat: ids_category,callcenter:ids_callcenter, campaign: ids_campaign}).done(function(response){
    if (response != undefined){
      if(response.msj_error == true){
        alert("No data in the date range.");
      }
